...
patch -p1 < patches/doc.patch
echo "1.5 78356097638ef2ed19eadb068cff88580b2a362b https://github.com/LLNL/ldms-plugins-llnl.git" > VERSION
...
#
# first version uses rpath but may not work in rpm packaging w/destdir builds
#eval CFLAGS=-I$ovis_ldms_includedir LDFLAGS="\"-L$ovis_ldms_libdir -Wl,-rpath,$ovis_ldms_libdir\"" ../configure $(cat $ovis_ldms_pkglibdir/ovis-ldms-configure-args) --prefix=$ovis_ldms_prefix --with-ldms-libdir=$ovis_ldms_libdir && make && make install
# second version ignores rpath but needs ld_library_path correct at runtime
export OVIS_DOC_DIR=$ovis_ldms_docdir-$ovis_ldms_version
eval CFLAGS=-I$ovis_ldms_includedir LDFLAGS=-L$ovis_ldms_libdir ../configure $(cat $ovis_ldms_pkglibdir/ovis-ldms-configure-args) --prefix=$ovis_ldms_prefix --with-ldms-libdir=$ovis_ldms_libdir && make && make install
